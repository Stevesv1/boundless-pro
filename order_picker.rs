    ...
    P: Provider<Ethereum> + 'static + Clone + WalletProvider,
{
    ...
    async fn evaluate_lockable_order(
        ...
        // Check if the current time is before biddingStart to lock immediately, bypassing profit checks
        if now < order.request.offer.biddingStart {
            // Lock the order immediately before bidding starts as per requirement
            // Bypasses profit checking since the contract does not enforce timing restrictions
            tracing::info!(
                "Locking order {} immediately since current time ({}) is before biddingStart ({})",
                order.id(),
                now,
            ...
            return Ok(Lock {
                ...
                target_timestamp_secs: 0, // Lock ASAP
                expiry_secs: order.request.offer.biddingStart + order.request.offer.lockTimeout as u64,
            });
        }
    
        // Existing logic for when current time is at or after biddingStart
